...
describe('settingDivisionValue.vue', () => {
  ...
  it("구분값을 변경하면 각각의 메시지 건수가 구분값에 맞는 값이 등록된다.", async () => {
    ...
    const wrapper = mount(SettingDivisionValue,{
      localVue,
      vuetify:new Vuetify()
    });
    ...
    const inputs = wrapper.findAll("[data-test='message-input']")
    await wrapper.vm.changeSelect(0);

    console.dir(inputs.at(0))
    //console.log(inputs.at(0).);
    expect(inputs.at(0)).toBe("1000건");
    expect(inputs.at(1).text()).toBe("1000건");
    expect(inputs.at(2).text()).toBe("1000건");
    expect(inputs.at(3).text()).toBe("1000건");
  })
